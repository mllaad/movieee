...
import { useGetDiscover, useGetImage } from "./usemovie";
import {
  Section,
  PageTitle,
  SearchBox,
...
const Movie = () => {
  const image = useGetImage(unsplashAPI.cinemaTwo());
  ...
  const isLoad = status === "fulfilled" ? true : false;
  return (
    <Section>
      <Background bg={image} />
      <PageTitle>
      ...
      <Grid>
        {isLoad ? (
          results.map((result, i) => (
            <AdvancedCard products={result} type={"movie"} key={i} />
          ))
        ) : (
